...
ff = open("output_labels/output_Dr_Gaurav.pkl.json")
data = json.load(ff)
print(len(data))
#print(data[0].keys())
valid_dir = "data/valid/JPEGImages/Dr_Gaurav_Varshney_0deg_LeftTilt_endo/"
train_dir = "data/train/JPEGImages"
train_annotations= "data/annotations/instances_val_sub.json"
save_dir = "data/Dr_Gaurav_Varshney_0deg_LeftTilt_endo.avi/"
os.mkdir(save_dir)
...
def apply_mask(image, mask, color, alpha=0.5):
    ...
    """
    #mask = cv2.resize(mask,(720,540), interpolation = cv2.INTER_NEAREST)
    #image = cv2.resize(image,(1024,1280), interpolation = cv2.INTER_NEAREST)
