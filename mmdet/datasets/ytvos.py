...
class YTVOSDataset(CustomDataset):
    ...
    def prepare_train_img(self, idx):
        ...
        #writing bboxes
        # save_path = ("/scratch/cse/phd/anz148198/MaskTrackRCNN_output/test_output/")
        # instrument_path = vid_info['filenames'][frame_id].split("/")[0]
        # if not osp.exists(save_path):
        #     os.makedirs(save_path)
        # if not osp.exists(osp.join(save_path, instrument_path)):
        #     os.makedirs(osp.join(save_path, instrument_path))
        # img_name=vid_info['filenames'][frame_id].split("/")[-1].split(".")[0]
        ...
        # # ref_img_name=vid_info['filenames'][ref_frame_id].split("/")[-1].split(".")[0]
        # # #print("ref_img_name", ref_img_name)
        # save_img = img_name+".png"
        # # #save_img_path = osp.join(save_path, instrument_path)
        # save_img_path = osp.join(save_path, instrument_path)+"/"+save_img
        #print("save_img_path", save_img_path)
        if (self.flip_ratio !=0.0):
            blurKernelSize = int(self.flip_ratio*100)
            img = cv2.GaussianBlur(img,(blurKernelSize,blurKernelSize),0)
            #cv2.imwrite("blurred_img.png",img)
            ref_img = cv2.GaussianBlur(ref_img,(blurKernelSize,blurKernelSize),0)
            #cv2.imwrite("blurred_ref_img.png",ref_img)
        ...
        gt_ids = ann['obj_ids']
        ref_lbls = ref_ann['labels']
        #print("ref_ids", ref_ids)
        #print("gt_ids", gt_ids)
        #print("ref_lbls", ref_lbls)
        ...
        img_meta = dict(
            ...
            # cur_img_path= osp.join(self.img_prefix, vid_info['filenames'][frame_id]),
            # curr_ref_img_path = osp.join(self.img_prefix, vid_info['filenames'][ref_frame_id]),
            # ref_curr_lbl = ref_lbls)
    ...
    def prepare_test_img(self, idx):
        ...
        save_path = ("/scratch/cse/msr/siy207566/")
        instrument_path = vid_info['filenames'][frame_id].split("/")[0]
        if not osp.exists(save_path):
            os.makedirs(save_path)
        if not osp.exists(osp.join(save_path, instrument_path)):
        ...
        img_name=vid_info['filenames'][frame_id].split("/")[-1].split(".")[0]
        save_img = img_name+".png"
        save_det_img = img_name+"_det_"+".png"
        # #save_img_path = osp.join(save_path, instrument_path)
        save_img_path = osp.join(save_path, instrument_path)+"/"+save_img
        save_det_img_path = osp.join(save_path, instrument_path)+"/"+save_det_img
        #blurKernelSize = int(self.flip_ratio*100)
        img = mmcv.imread(osp.join(self.img_prefix, vid_info['filenames'][frame_id]))
        #img = cv2.blur(img,(blurKernelSize,blurKernelSize))
        ...
        def prepare_single(img, frame_id, scale, flip, proposal=None):
            ...
            _img_meta = dict(
                ...
                flip=flip,
                image_path= save_img_path,
                image_det_path = save_det_img_path, 
                cur_img_path= osp.join(self.img_prefix, vid_info['filenames'][frame_id]))
